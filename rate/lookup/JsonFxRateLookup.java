...
public class JsonFxRateLookup implements FxRateLookup {
    ...
    public JsonFxRateLookup(ObjectMapper objectMapper, @Value("${fxrates.dir}") String baseDir, @Value("${fxrates.lookback_days}") int lookbackDays) {
    ...
    public Optional<BigDecimal> findKrwPerUnit(String currency, LocalDate postingDate) {
        ...
        LocalDate day = postingDate; // day.minusDays(1)... 원본 지키자..나중을 위해

        for (int i = 0; i <= lookbackDays; i++) {
            Optional<DailyFxRates> dailyOpt = cache.computeIfAbsent(day, this::loadOneDayOptional);
    ...
    private Optional<DailyFxRates> loadOneDayOptional(LocalDate date) {
        try {
            ...
            throw new RuntimeException("해당 날짜 파일을 읽지 못함. 날짜: " + date, e);
        }
    }
}
